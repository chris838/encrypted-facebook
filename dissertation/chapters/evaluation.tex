...
The per-image theoretical capacity $C$ is a function of the number of bits available per image $A$\footnote{Determined by the implementation. Recall that all images are $720 \times 720$ pixels.} and the bit error probability $p_e$:
...
\begin{axis}[grid=major,
    xlabel=Recipient group size, ylabel=Encoding Time (seconds), height=8cm,width=8cm,
...
\begin{tikzpicture}
\begin{axis}[grid=major,
    xlabel=Recipient group size, ylabel=Decoding Time (seconds), height=8cm,width=8cm,
    ymin=0, xmin=0, xmax=400, legend style={legend pos=south west, empty legend}]
